...
class Dataset:
    ...
    class_index_to_name = get_dataset_classes(
        os.path.join("/content/VOCBirds/") + dsconverter.conv.obj_classes_filename)
    ...
    def __init__(self, split, image_preprocessing_params, compute_feature_map_shape_fn, feature_pixels=16,
        ...
        print(f"self.num_classes ${self.num_classes}, Dataset.num_classes ${Dataset.num_classes}")
        ...
        print(f"self.class_index_to_name {self.class_index_to_name} ?= Dataset.class_index_to_name {Dataset.class_index_to_name}")
