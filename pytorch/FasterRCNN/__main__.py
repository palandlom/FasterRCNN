...
def predict(model, image_data, image, show_image, output_path):
    ...
    if not show_image and output_path is not None:
        write_image_score_data(output_path, scored_boxes_by_class_index, voc.Dataset.class_index_to_name)
...
def write_image_score_data(output_image_file_path, scored_boxes_by_class_index, class_index_to_name):
    # TODO pal Dict[int, np.ndarray]
    #       Scored boxes, (N, 5) tensor of box corners and class score,
    #       (y1, x1, y2, x2, score), indexed by class index.
    #  save Scored in json to output_path

    output_prediction_data_path = os.path.splitext(output_image_file_path)[0] + "-score.json"
    print("Wrote image score data to '%s'" % output_prediction_data_path)

    image_prediction_data = {"image": output_image_file_path}
    objects = []
    for class_index, scored_boxes in scored_boxes_by_class_index.items():
        for i in range(scored_boxes.shape[0]):
            ...
            obj["class"] = class_index_to_name[class_index]
            obj["score"] = scored_box[4]
            obj["y_min"], obj["x_min"], obj["y_max"], obj["x_max"] = scored_box[0:4]
            if len(obj) > 0:
                objects.append(obj)
        image_prediction_data["objects"] = objects

    with open(output_prediction_data_path, "w") as write_file:
        json.dump(image_prediction_data, write_file, indent=4)
